...
var ver_ctrl = new Controller(0.3, 0.01, 0.1)
  ...
  , size_ctrl = new Controller(0.35, 0.01, 0.1)
  ;
  ...
  if(tracking && (!processingImage) && lastPng) {
    ...
    async.waterfall([
      ...
      function(faces, im, cb) {
        ...
        var faceArea;
        var trueArea = 5000;
        var lower_threshold = [0, 104, 0];
        var upper_threshold = [120,205 ,255];
       
	im.convertHSVscale();
        im.inRange(lower_threshold, upper_threshold); // skin detection
        ...
        for(var k = 0; k < faces.length; k++)
        {
            face = faces[k];
            var b = 0;
		for(var i=0;i<face.height;i++)
		{
			for(var j=0;j<face.width;j++)
			{
				//process.stdout.write(im.get(face.y+i,face.x+j)+ " ");
				if(im.get(face.y+i,face.x+j) > 0 )
				{
					b++;
				}
			}
		...
		var face_sz = face.width * face.height;
		console.log("ratio : "+b/face_sz);
		if( (!biggestFace || biggestFace.width < face.width) && (b>(face_sz/10))) 
		{
		  console.log("biggestFace");
			console.log("b : "+b+" size : "+(face.height*face.width));
		  biggestFace = face;
		}	   
        }
	...
	//console.log("b : "+b+" size : "+(face.height*face.width));
         io.sockets.emit('face', { x: face.x, y: face.y, w: face.width, h: face.height, iw: im.width(), ih: im.height() });
          ...
          var sizeAmount = -(trueArea - faceArea) / trueArea;
          ...
          sizeAmount   = size_ctrl.update(-sizeAmount);   // pid
          //console.log("turnAmount : ", turnAmount);
        ...
        /*  log( "  turning " + turnAmount );
           ...
           log( "  turning " + turnAmount );
            if (debug) io.sockets.emit('/message', 'turnAmount : ' + turnAmount);
            if( turnAmount < 0 ) client.right( Math.abs( turnAmount*10 ) );
            else client.left( turnAmount*10 );     

            log( "  sizing " + sizeAmount );
            if (debug) io.sockets.emit('/message', 'sizeAmount : ' + sizeAmount);
            if( sizeAmount < 0 ) client.front( Math.abs( sizeAmount ) );
            else                   client.back( sizeAmount);
